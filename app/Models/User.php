...
class User extends Authenticatable
{
    ...
    {
        if($userType == 1){
        $query= DB::select(DB::raw("SELECT pg.page_name,pg.page_title, tbl_modules.parent_id,tbl_modules.`id` AS module_id ,(SELECT md.`name` FROM tbl_modules md WHERE md.id = tbl_modules.parent_id) AS module
                        FROM tbl_modules
