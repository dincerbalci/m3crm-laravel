            ...
            <div class="intro-y box">
                <div class="flex flex-col sm:flex-row items-center p-5 border-b border-slate-200/60 dark:border-darkmode-400">
                    <h2 class="font-medium text-base mr-auto">
                        Complaint By TAT Report
                    </h2>
                </div>
                <form id='complaint_form_search' method="GET" action="{{ route('report_complaint_tat') }}" >
                <div id="inline-form" class="p-5">
                        ...
                        <div >
                            <label for="regular-form-1" class="form-label">Status</label>
                                <select data-placeholder="Select your favorite actors"  name='status' class="tom-select w-full tomselected" id="tomselect-1" tabindex="-1" hidden="hidden">
                                    ...
                                    <option value="1" {{"1" == request()->status ? 'Selected' : ''}}>Open</option>
                                </select>
                        </div>
                            <div class="">
                    ...
                    <a href="{{route('report_complaint_tat')}}"  class="btn btn-inverse mt-5">Reset</a>
        ...
        <div class="overflow-x-auto scrollbar-hidden">
            <div class="intro-y overflow-auto lg:overflow-visible mt-8 sm:mt-0">
                <table class="table table-report sm:mt-2">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">Total Open</th>
                            <th class="whitespace-nowrap">Within TAT</th>
                            <th class="whitespace-nowrap">Beyond TAT</th>
                        </tr>
                    </thead>
                    <tbody>
                        @for($i=0; $i < count($companintTat); $i++)
                        <tr class="intro-x">
                            <td class="text-left">{{ $companintTat[$i]->total}}</td>
                            <td class="text-left">{{ $companintTat[$i]->within_tat}}</td>
                            <td class="text-left">{{ $companintTat[$i]->beyond_tat}}</td>
                        </tr>
                        @endfor
                    </tbody>
                </table>
