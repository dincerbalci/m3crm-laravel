    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 lg:col-span-6">
            ...
            <div class="intro-y box">
                ...
                <form  method="POST" action="{{ route('unit.update',$unit->id) }}" onsubmit="return check()">
                    @method("PUT")
                    @csrf
                <div id="inline-form" class="p-5">
                    <div class="preview">
                            ...
                            <div >
                                <label for="regular-form-1" class="form-label">Group  <span style="color: red">*</span></label>
                                    <select data-placeholder="Select your favorite actors" id="group_id" name="group_id" class="tom-select w-full tomselected" id="tomselect-1" tabindex="-1" hidden="hidden" required>
                                        ...
                                        <option value="{{$group[$i]->id}}" {{$unit->group_id == $group[$i]->id ? "Selected" : ""}} >{{ucwords($group[$i]->group_name)}}</option>
                                        @endfor
                                    </select>
                            </div>
                            <div >
                                <label for="regular-form-1" class="form-label">Region  <span style="color: red">*</span></label>
                                    <select data-placeholder="Select your favorite actors" name="region_id" class="tom-select w-full tomselected" id="tomselect-1" tabindex="-1" hidden="hidden" required>
                                        ...
                                        <option value="{{$region[$i]->id}}" {{$unit->group_id == $region[$i]->id ? "Selected" : ""}}>{{ucwords($region[$i]->region_name)}}</option>
                                        @endfor
                                    </select>
                            </div>
                            <div>
                            <label for="regular-form-1" class="form-label">Unit Name <span style="color: red">*</span></label>
                            <input type="text" class="form-control col-span-4" value="{{$unit->unit_name}}" name="unit_name" placeholder="Unit Name" aria-label="default input inline 2" required>
                        </div>
                        <div>
                            <label for="regular-form-1" class="form-label">Branch Code <span style="color: red">*</span></label>
                            <input type="text" class="form-control col-span-4" value="{{$unit->branch_code}}" name="branch_code" placeholder="Branch Code" aria-label="default input inline 2" required>
                        </div>
                        <div>
                            <label for="regular-form-1" class="form-label">Address</label>
                            <input type="text" class="form-control col-span-4" value="{{$unit->address}}" name="address" placeholder="Address" aria-label="default input inline 2" required>
                        </div>
                        <div>
                            <label for="regular-form-1" class="form-label">E-Mail Address <span style="color: red">*</span></label>
                            <input type="text" class="form-control col-span-4" value="{{$unit->email}}" name="email" placeholder="E-Mail Address" aria-label="default input inline 2" required>
                        </div>
                        <div class="form-check form-switch sm:mt-0" style="margin-top: 17px;margin-bottom: 10px;">
                            ...
                            <input id="show-example-3" data-target="#header" {{$unit->is_active == '1' ? "checked" : ""}} name="is_active" class="show-code form-check-input mr-0 ml-3" type="checkbox" >
                        </div>
                            ...
                            <div class="mt-3" >
                                <label for="regular-form-1" class="form-label">Escalation Time Period </label>
                                    <select  class="tom-select w-full tomselected" name="escalation_time1" id="escalation_time1"  tabindex="-1" hidden="hidden">
                                        ...
                                        @for($i = 1; $i <= $tatSave; $i++)
                                        <option value="{{$i}}" {{$escalationBranches[0]->escalation_time1 == $i ? "Selected" : ""}}>{{$i}} Days</option>
                                        @endfor
                                    </select>
                            </div>
                            @php $level1=explode(',',$escalationBranches[0]->level1); @endphp
                            <div class="mt-3" >
                                <label for="regular-form-1" class="form-label">Level 1</label>
                                    <select  class="tom-select w-full tomselected" name="level1[]" id="level1" multiple="multiple">
                                        @for($i=0; $i < count($escalationGroup); $i++)
                                            ...
                                            <option value="{{$escalationGroup[$i]->id}}" {{$level1[$j] == $escalationGroup[$i]->id ? 'Selected' : ''}} >{{ ucwords($escalationGroup[$i]->group_name) }}</option>
                                            @endfor
                                        @endfor
                                    </select>
                            </div>
                        ...
                        <div class="mt-3" >
                            <label for="regular-form-1" class="form-label">Escalation Time Period </label>
                                <select  class="tom-select w-full tomselected " name="escalation_time2" id="escalation_time2" tabindex="-1" hidden="hidden">
                                    <option value="0" selected="true">Select Time Period</option>
                                        @for($i = 1; $i <= $tatSave; $i++)
                                        <option value="{{$i}}" {{$escalationBranches[0]->escalation_time2 == $i ? "Selected" : ""}}>{{$i}} Days</option>
                                        @endfor
                                    
                                </select>
                        </div>
                        @php $level2=explode(',',$escalationBranches[0]->level2); @endphp
                        ...
                        <div class="mt-3" >
                            ...
                            <select  class="tom-select w-full tomselected" name="level2[]" id="level2" multiple="multiple">
                                @for($i=0; $i < count($escalationGroup); $i++)
                                    ...
                                    <option value="{{$escalationGroup[$i]->id}}" {{$level2[$j] == $escalationGroup[$i]->id ? 'Selected' : ''}} >{{ ucwords($escalationGroup[$i]->group_name) }}</option>
                                    @endfor
                                @endfor
                            </select>
                        </div>
                    ...
                    <div class="mt-3" >
                        <label for="regular-form-1" class="form-label">Escalation Time Period </label>
                            <select  class="tom-select w-full tomselected " id="escalation_time3" name="escalation_time3" tabindex="-1" hidden="hidden">
                                <option value="0" selected="true">Select Time Period</option>
                                @for($i = 1; $i <= $tatSave; $i++)
                                <option value="{{$i}}" {{$escalationBranches[0]->escalation_time3 == $i ? "Selected" : ""}}>{{$i}} Days</option>
                                @endfor
                                
                            </select>
                    </div>
                    @php $level3=explode(',',$escalationBranches[0]->level3); @endphp
                        ...
                        <label for="regular-form-1" class="form-label">Level 3</label>
                            <select  class="tom-select w-full tomselected" name="level3[]" id="level3" multiple="multiple">
                                @for($i=0; $i < count($escalationGroup); $i++)
                                    ...
                                    <option value="{{$escalationGroup[$i]->id}}" {{$level3[$j] == $escalationGroup[$i]->id ? 'Selected' : ''}} >{{ ucwords($escalationGroup[$i]->group_name) }}</option>
                                    @endfor
                                @endfor
                            </select>
                    </div>
                ...
                <div class="mt-3" >
                    <label for="regular-form-1" class="form-label">Escalation Time Period </label>
                        <select  class="tom-select w-full tomselected " name="escalation_time4" id="escalation_time4" tabindex="-1" hidden="hidden">
                            <option value="0" selected="true">Select Time Period</option>
                                @for($i = 1; $i <= $tatSave; $i++)
                                <option value="{{$i}}" {{$escalationBranches[0]->escalation_time4 == $i ? "Selected" : ""}}>{{$i}} Days</option>
                                @endfor
                            
                        </select>
                </div>
                @php $level4=explode(',',$escalationBranches[0]->level4); @endphp
                ...
                <div class="mt-3" >
                    <label for="regular-form-1" class="form-label">Level 4</label>
                        <select  class="tom-select w-full tomselected" name="level4[]" id="level4" multiple="multiple">
                            @for($i=0; $i < count($escalationGroup); $i++)
                                @for($j=0; $j < count($level4); $j++)
                                <option value="{{$escalationGroup[$i]->id}}" {{$level4[$j] == $escalationGroup[$i]->id ? 'Selected' : ''}} >{{ ucwords($escalationGroup[$i]->group_name) }}</option>
                                @endfor
                            @endfor
                        </select>
                </div>
